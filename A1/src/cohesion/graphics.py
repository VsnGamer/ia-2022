...
def draw_board_surface(board: BoardState, surface: pygame.Surface):
    ...
    for piece in board.pieces:
        for x, y in piece.positions:
            pygame.draw.rect(surface, piece.color.get_color_rgb(), (x * CELL_SIZE, y * CELL_SIZE, CELL_SIZE, CELL_SIZE))
    for x in range(board.width):
        for y in range(board.height):
            pygame.draw.rect(surface, (255, 255, 255), (x * CELL_SIZE, y * CELL_SIZE, CELL_SIZE, CELL_SIZE), 1)
...
def board_properties_text(board: BoardState):
    ...
    text = font.render(f"Colors: {board.get_num_colors()}", True, (255, 255, 255))
    surface.blit(text, (0, 0))

    text = font.render(f"Pieces: {board.get_num_pieces()}", True, (255, 255, 255))
    surface.blit(text, (0, 20))

    # Win: true/false, the boolean text should be red if false and green if true
    text = font.render(f"Win: {board.is_win()}", True, (255, 0, 0) if not board.is_win() else (0, 255, 0))
